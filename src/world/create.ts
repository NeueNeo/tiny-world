...
export function createCreature(type: Creature['type'], worldWidth: number, worldHeight: number): Creature {
  ...
  const sizes: Record<Creature['type'], number> = { bug: 4, snail: 6, butterfly: 5, caterpillar: 5, ant: 2.2, dragonfly: 6, bee: 3.75 };
  
  return {
    id: randomId(),
    pos: { 
      x: randomInRange(2, worldWidth - 2), 
      y: randomInRange(2, worldHeight - 2) 
    },
    vel: { x: 0, y: 0 },
...
export function createWorld(width: number, height: number): World {
  ...
  for (let patch = 0; patch < 6; patch++) {
    const patchCenterX = randomInRange(5, width - 5);
    const patchCenterY = randomInRange(5, height - 5);
    ...
    for (let m = 0; m < mushroomsInPatch; m++) {
      ...
      const pos = {
        x: Math.max(1, Math.min(width - 1, patchCenterX + offsetX)),
        y: Math.max(1, Math.min(height - 1, patchCenterY + offsetY))
      };
  ...
  for (let patch = 0; patch < 7; patch++) {
    const patchCenterX = randomInRange(10, width - 10);
    const patchCenterY = randomInRange(10, height - 10);
